...
| 文件 | 内容 | 大致条目数 |
|---|---|---|
| `weapons.json` | **武器**（主武器 / 副武器 / 投掷物）及其数值、特性、tips | 103 |
...
| `terms.json` | **术语表** —— 全站译名的**唯一真相源**（见 §4） | 1697 |
...
> 字段名与顺序**逐个核对过**：来自 `SCHEMA.md` §3 / §7.3 与文件里真实的 103 条（2026-09-23 实测）。
> 真实顺序是
...
- **`description` / `lore` / `variants` / `tips`**：都是必填字段。
  实测 `description` 是英文简介（103 条里 101 条有内容）；`lore` 103 条**全是空字符串 `""`**；
  `variants` 与 `tips` 103 条**全是空数组 `[]`**。照这个现状写即可，**不能删字段**。
  > ⚠ 2026-09-23 起：新条目的 `description` 一律填上游 `{{Quote|…|Armory Description}}` 的英文原文。
  > 历史上有 **33 条** `description` 是抓取残留（Attachments 表那句 `Any math should be done…`）、
  > **38 条**是版本号戳，**41 条 `description_zh` 是照着那段残留翻译的** —— 别照着老条目抄这两列。
...
- **`related`**：⚠️ 实测 103 条**全部是空对象 `{}`**（不是数组！）。
  这是 `SCHEMA.md` §8 登记在案的历史不一致（canonical 是字符串数组，本文件未迁移）。
  **照抄 `{}` 即可，不要自作主张改成 `[]`** —— 否则你会变成全文件唯一一个异类。
- **`detailed_stats`**：详细面板，形态很杂。实测有 **19 条是 `null`**（上游该页还没出数值表，如挂在 `Weapons_Needing_Stats` 下），其余至少含 `"attacks"`。
  攻击元素形如 `{ "name": "…", "type": "…", "projectile": {…}, "damage": {…}, "penetration": {…}, "special_effects": {…} }`，
  `type` 实测有 `projectile` / `explosion` / `status` / `damage` / `spray`。
  **最省事的做法：整段照抄一把同类武器的 `detailed_stats`，只改里面的数值** —— 但**上游没有就写 `null`，不要编**。
- **`icon`**：⚠️ **必须用站内相对路径** `./assets/weapons/<文件名>.png`，图片随 PR 放进 `HD2_Wiki/assets/weapons/`。
  **不要贴 wiki.gg 链接**：本文件现有 103 条**全部**是热链，那是登记在案的历史例外、**额度正好用满 103**
  （`scripts/validate_wiki_data.py` 的 `KNOWN` 基线与之同步）——
  你**新增**一条热链，`IMG.HOTLINK` 就会超过基线上限，CI 直接变红。
  > 从上游 infobox 取图时注意 `image` 参数可能是**未展开的 `{{PAGENAME}} …Render.png`**，
  > 要先把 `{{PAGENAME}}` 换成页面标题再拼 240px thumb URL，并对 URL 做一次 200 校验。
- **别忘了**：顶层 `"total"` 要 **+1**（当前 103 → 104）。
