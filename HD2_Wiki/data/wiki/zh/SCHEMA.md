...
3. 一图流是**文字密集图**：有损压缩会把小字压糊，`quality` 从 **0.9** 起试；优先「降分辨率上限」而不是继续降质量。
   - **2026-09-21 批量复测（12 张新母版，均 2560×1440，`canvas` `toBlob("image/webp", 0.9)`）**：得 **539 680 – 726 164 B（0.51–0.69 MB）**、全图 **PSNR 36.04–37.39 dB**，与首批 4 张（q=0.9，601 896 – 718 808 B）**同档**；对其中 PSNR 最低的一张（`predator_stalker`，36.04 dB）做**最高细节区块 3 倍最近邻放大**目视，小字无糊化、无歧义 → **维持 q=0.9**。
   - **不要凭「质量优先」自行上调**：实测 q=0.92 相对 q=0.9 仅 **+0.2 dB**，体积却多约 **10%**；q=1.0 直接膨胀到 **2.37 MB**（见 §9 第 19 条）。一图流的收益在**分辨率**，不在质量分。
...
### 7.15 `search_index.json` / `search_entries.json`（站内搜索索引 · 2026-09-23 登记）

**生成物，不要手改**：`python scripts/gen_search_index.py`（幂等；数据没变则不写盘）。
首页 `wiki.html` 的搜索框与「🎲 开始探索（随机条目）」都吃这两个文件。
...
3. **小节级条目的锚点键取主干的 `id`，中文覆盖的 `subsections_zh` / `items_zh` 也要收**。
   ...
   这类"同一结构在主干与覆盖里字段名不同源"的漏收集，只能**拿正文里真实的一句话去搜**才能验出来 ——
   真数据全绿、体积也正常，不会报任何错。
4. **前端一律不要用 `cache: "force-cache"` 拉这两个文件**（2026-09-24 实测踩坑）：
   文件刚推上去时边缘可能仍返回 404，而 `force-cache` 会把那次 **404 永久钉在浏览器缓存里** ——
   同一 URL 之后永远 404（`no-store` 与 `?cb=` 都正常 200），表现是「随机条目点了没反应、
   标题联想永远为空」，硬刷新之前不自愈。正确做法：默认缓存 + 失败时用 `?cb=<时间戳>` 绕缓存重试一次；
   并且**随机条目必须有出口**（取不到就提示并退回板块导航），绝不递归重试。

自动化：`.github/workflows/build-search-index.yml`（数据 push 触发 + 手动）重建并提交；
`fetch-patchnotes.yml` 抓完公告后也会重建；`validate-data.yml` 有一项**不阻塞**的新鲜度提示
（过期会在 Job Summary 里显式标出，但不把 CI 改红 —— 机翻流水线用 Contents API 直接推数据时索引会短暂落后）。
...
| `blocks/navigation.json` 的卡片 `count`（2026-09-22 实测，**线上一致**） | 与数据集 `total` 不一致：`武器` **45**（数据集实际 103）、`战略配备` **89**（实际 109）、`敌人` **65**（实际 95）；其余对得上（强化资源 20 / 战争债券 25 / 任务 105 / 游戏机制 5 / 更新公告 10） | 目录卡片计数应与对应数据集 `total` 一致（§2 的 `total` 约定、§10 第 8 条自检项） | **已修（2026-09-24）**：`武器` **45 → 103**、`战略配备` **89 → 115**、`敌人` **65 → 85**（`战术搭配器` 89 本就对得上 `loadout.json`）。`敌人` 的取值口径定为 **页面展示口径**：数据集 95 条里有 10 条是变种/亚阵营的容器条目，`enemies.html` 实际渲染 85 张卡、页头也写「共 85 种敌人 · 10 个变种」，目录卡片与它链接的页面**必须一致**，否则点进去就自相矛盾。校验器仍**不**校验该字段 —— 改数据集的条目数时，**要顺手改这里的 `count`**（这是本站「同一事实存两份」的已知点之一） |
...
21. **`weapons.json` 补齐 §19.15 列出的另外 12 条（2026-09-23 登记并实施）**：`91 → 103 条`（primary 50→55 / secondary 21→25 / throwables 20→23），校验器 `IMG.HOTLINK` 基线 91 → 103。
    - **两类缺口要分清（别混成一句"漏抓"）**：
      - **A 类 · 同根因的 `/` 过滤受害者（5 条）**：`SMG/FLAM-34 Stoker`、`G/SH-39 Shield`、`M6C/SOCOM Pistol`、`P/40-K Bolt Pistol`、`R/40-K Hot-Shot Marksman Rifle` —— 与 §9 第 20 条同因，标题含 `/` 被 2026-08 的抓取循环跳过。
      - **B 类 · 2026-08 之后上游新增（7 条）**：`AR-11 Arbitrator`、`CQC-73 Entrenchment Tool`、`G-60 Anti-Tank Seeker`、`G-8 Immolation`、`GL-15 Evictor`、`LAS-12 Sai`、`P-34 Breacher` —— 它们**不是**过滤器漏掉的，是当时还不存在；判据是上游 `Change History` 的 Addition 版本（`1.007.100` / `1.006.100`）晚于首次抓取。
      - ⚠ 全量缺口的**正确查法**：拿上游 `Category:Primary Weapons` / `Secondary Weapons` / `Throwables`（含 Special/Standard 子类）的 `categorymembers` 与 `weapons.json` 的 `name_en` 做**集合差** —— L2 日报只看"公告提到过的实体"，查不出从未被公告提及的条目（本次集合差给出 18 条，去掉 2 条 `Community:Aprilfools/` 与 4 条 `Category:Support Weapons`（属 `stratagems_full.json`）后正好 12 条）。
    - **`detailed_stats` 允许为 `null`（本次 +7 条，全库 19 条）**：上游这几页还挂着 `Weapons_Needing_Stats`，**没有数值表**（`attacks` 为空）→ 一律写 `null`，`weapon.html` 的 `renderDetailedStats()` 遇 `null` 直接不渲染「⚙️ 详细数据」区块。**不要为了好看去编数值表**。
    - **`stats_short.penetration` 的来源与陷阱**：该字段只在旧编码器能从 infobox 渲染值里认出 `Light/Medium/Heavy` 时才正确，遇到 `Anti-Tank I/III`、`Unarmored`、`No Hitbox` 或**多值**（`Heavy (Projectile) Medium (Explosion)`）会**静默回退成 `light`**。既有 91 条里 `Anti-Tank *` 共 8 条也都是 `light`（历史一致性）。本次**只改一处**：`P/40-K Bolt Pistol` 的 `heavy`（infobox 渲染值明写 `Heavy (Projectile)`，且其 traits 就是 `Heavy Armor Penetrating`）。**判据**：先把全库 `detailed_stats` 首段 `pen_direct` 与 `stats_short.penetration` 交叉统计，再逐条回上游看渲染值 —— 别用"AP 数字 → 轻/中/重"自行发明映射。
    - **`warbonds.json` 的页序错误（本次一并修正）**：`ironclad_democracy` 表里 `G-8 Immolation`（2→1）、`P-34 Breacher`（3→2）、`GL-15 Evictor`（2→3）**页号与上游不符** —— 上游**渲染页**（`prop=text`，按 `<h2>Page N</h2>` 切段）与 **wikitext**（`==Page N==` 小节）两种读法一致，且同页的非武器奖励（`BFM-16 Tanker` / `BFM-220 Ironclad` / `Helmet Durability Check`）两边对得上 → 判定为站内表错。**修页序前先用两种读法交叉验证**，别只信一种。
    - **`icon` 取图陷阱**：新页 infobox 的 `image` 参数可能是**未展开的 `{{PAGENAME}} Throwable Render.png`**（G-8 / G-60 都是）→ 必须先把 `{{PAGENAME}}` 替换成页面标题，再拼 `…/thumb/<File>/240px-<File>` 并对 URL 做 200 + PNG 魔数校验（本次这两张第一次就 404，替换后 200）。
    - **译名口径（2026-09-23 用户已复核）**：站内**没有任何**这 12 个型号的中文名来源 —— 已全仓扫描（175 个文本文件）、读了私密仓的 `术语表.txt`（409 行，只有通用术语）、查了 `patchnotes_zh.json`（这几条在机翻正文里**仍是英文**）。故按「拟好 + 列出问用户」的规程（§18.3-A 先例）提交复核，结果：
...
25. **站内全文搜索（2026-09-23 登记并实施）**：首页搜索改为**段落级全文检索**，
    数据来自预生成的确定性索引（见 §7.15），前端 `wiki.html` 惰性加载 + 键盘可用（↑/↓/Enter/Esc）；
    「🎲 开始探索（随机条目）」从全部条目里随机跳一条（含机制页/公告的**小节深链**）。
    判据（改这块时必须逐条过）：
    ① 拿**正文里真实存在的一句话**去搜，必须命中**带 `#锚点` 的那一条**（不是只命中父页面）；
    ② 英文型号能搜到对应中文条目（`t` 字段同时存中英名）；
    ③ 索引 `--check` 必须为 0（与数据一致）；
    ④ 新增数据集时 `SOURCES` 与 `FORMAT` 同步更新（见 §7.15）。
