...
OUT = os.path.join(BASE, "weapons.json")
d = json.load(open(OUT, encoding="utf-8"))
items = d["weapons"]
print(f"总 {len(items)} 个武器，开始获取 detailed_stats...", flush=True)
...
if fails:
    print("null 列表:", ", ".join(fails[:20]), flush=True)